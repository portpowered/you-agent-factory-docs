/**
 * Discoverability proof for rewritten /docs/concepts/tokens (story 007).
...
const DOCS_SLUG = ["concepts", "tokens"] as const;
const DOCS_ROUTE = "/docs/concepts/tokens";
const DOCS_TITLE = "Tokens";
...
describe("tokens concept discoverability (007)", () => {
  afterEach(() => {
    cleanup();
  });

  test("default-locale messages stay model-inference focused", () => {
    expect(enMessages.title).toBe(DOCS_TITLE);
    expect(enMessages.description).toBe(DOCS_DESCRIPTION);
    expect(enMessages.description).toMatch(/Model-inference tokens/i);
    expect(enMessages.description).not.toMatch(
      /Petri|Colored Petri|work token glossary/i,
    );
