/**
 * Server helper: load the live Factory `Workstation` definition for the
 * workstations family index embed (W07 SchemaReference addressed mode).
 *
...
export const WORKSTATION_BASE_DEFINITION_POINTER =
  "/$defs/Workstation" as const;

const FACTORY_SCHEMA_RELATIVE_PATH =
  "node_modules/@you-agent-factory/api/generated/schemas/factory.schema.json";

export type WorkstationBaseSchemaEmbedModel = {
...
export function loadWorkstationBaseSchemaEmbedModel(): WorkstationBaseSchemaEmbedModel {
  ...
  const address: SchemaAddress = {
    publicArtifactId: artifact.specifier,
    pointer: WORKSTATION_BASE_DEFINITION_POINTER,
  };
  const definition = normalized.definitions.find(
    (entry) => entry.address.pointer === WORKSTATION_BASE_DEFINITION_POINTER,
  );

  if (!definition) {
    throw new Error(
      `Factory schema is missing Workstation base definition at ${WORKSTATION_BASE_DEFINITION_POINTER}`,
    );
  }
