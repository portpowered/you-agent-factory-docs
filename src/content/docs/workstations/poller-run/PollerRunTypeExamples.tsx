import { T } from "@/features/docs/components/T";
import {
  POLLER_RUN_TYPE_MINIMAL_EXAMPLE,
  POLLER_RUN_TYPE_MISUSE_POLLER_BEHAVIOR_COLLAPSE_EXAMPLE,
} from "./poller-run-type-examples";

function formatExample(value: unknown): string {
  return `${JSON.stringify(value, null, 2)}\n`;
}

/**
 * Authored minimal and misuse examples for type POLLER_RUN.
 * JSON payloads stay outside message auto-link so field names remain literal.
 */
export function PollerRunTypeExamples() {
  return (
    <div className="min-w-0 space-y-4" data-poller-run-type-examples="">
      <div data-poller-run-type-example="minimal">
        <p>
          <T k="links.minimalExampleLabel" />
        </p>
        <pre>
          <code>{formatExample(POLLER_RUN_TYPE_MINIMAL_EXAMPLE)}</code>
        </pre>
      </div>
      <div data-poller-run-type-example="misuse-poller-behavior-collapse">
        <p>
          <T k="links.misuseExampleLabel" />
        </p>
        <pre>
          <code>
            {formatExample(
              POLLER_RUN_TYPE_MISUSE_POLLER_BEHAVIOR_COLLAPSE_EXAMPLE,
            )}
          </code>
        </pre>
        <p>
          <T k="links.misuseNote" />
        </p>
      </div>
    </div>
  );
}
