/**
 * Browser verify for techniques collection PF-L-strip (Related / References footer).
 *
 * Proves representative techniques pages (ralph + planner-executor) render
 * teaching content without trailing Related / References / RelatedDocs footer
 * chrome, while teaching sections (what-it-is / why-it-matters / how-it-works /
 * compared-to-nearby-techniques) remain visible. Does not invent a replacement
 * related surface or create planner-executor-in-action.
 *
 ...
 * `next dev --webpack`. Prefer `TECHNIQUES_RELATED_STRIP_PROBE_BASE_URL` when a
...
const PORT = Number(process.env.TECHNIQUES_RELATED_STRIP_PROBE_PORT ?? "3572");
const READY_TIMEOUT_MS = 180_000;
const PAGE_TIMEOUT_MS = 180_000;
const EXISTING_BASE_URL =
  process.env.TECHNIQUES_RELATED_STRIP_PROBE_BASE_URL?.trim();
...
const ROUTES: readonly RouteProbe[] = [
  {
    ...
    warmNeedle: 'id="what-it-is"',
    requiredHeadingNames: TEACHING_HEADINGS,
    requiredSectionIds: TEACHING_SECTION_IDS,
  },
  {
    path: "/docs/techniques/planner-executor",
    warmNeedle: 'id="what-it-is"',
    requiredHeadingNames: TEACHING_HEADINGS,
    requiredSectionIds: TEACHING_SECTION_IDS,
  },
  ...
  try {
    ...
    for (const route of ROUTES) {
      ...
      const probe = await page.evaluate(
        ({ requiredHeadingNames, requiredSectionIds }) => {
