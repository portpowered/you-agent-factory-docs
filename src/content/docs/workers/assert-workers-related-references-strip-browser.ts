/**
 * Browser verify for workers collection PF-L-strip (Related / References footer).
 *
 * Proves representative workers routes render teaching content without trailing
 * Related / References / RelatedDocs footer chrome:
 * - `/docs/workers/agent`: How To Use / Schema reference / Examples + AGENT_RUN
 *   companion teaching link inside How To Use
 * - `/docs/workers` family index: How To Use / selection / shared fields /
 *   worker schema + Factory schema teaching link
 *
 ...
 * `next dev --webpack`. Prefer `WORKERS_RELATED_STRIP_PROBE_BASE_URL` when a
...
const PORT = Number(process.env.WORKERS_RELATED_STRIP_PROBE_PORT ?? "3573");
const READY_TIMEOUT_MS = 180_000;
const PAGE_TIMEOUT_MS = 180_000;
const EXISTING_BASE_URL =
  process.env.WORKERS_RELATED_STRIP_PROBE_BASE_URL?.trim();
...
const ROUTES: readonly RouteProbe[] = [
  {
    path: "/docs/workers/agent",
    warmNeedle: 'id="how-to-use"',
    requiredHeadingNames: ["How To Use", "Schema reference", "Examples"],
    requiredSectionIds: ["how-to-use", "schema-reference", "examples"],
    teachingLink: {
      href: "/docs/workstations/agent-run",
      namePattern: "Agent-run workstation",
    },
  },
  {
    path: "/docs/workers",
    ...
    teachingLink: {
      ...
      namePattern: "Full Factory schema",
    },
  },
];
