...
const PORT = Number(process.env.GOAL_CHILD_REFERENCE_PROBE_PORT ?? "3521");
const PAGE_PATH = "/docs/references/packaged-factories-index/goal";
...
try {
  ...
  console.log(
    "PASS: packaged-factories-index/goal child reference browser verify",
  );
} catch (error) {
  console.error("Goal child reference browser verify failed:");
  console.error(error);
  process.exitCode = 1;
} finally {
  cleanup();
