/**
 * Browser verify for /docs/references/packaged-factories-index/fusion concise
...
const PORT = Number(process.env.FUSION_CHILD_REFERENCE_PROBE_PORT ?? "3522");
const PAGE_PATH = "/docs/references/packaged-factories-index/fusion";
...
try {
  ...
  if (normalized.includes('"id": "builtin-fusion"')) {
    throw new Error("Child page must not embed unabridged factory.json");
  }

  if (normalized.includes("review.factory-recording.v1.json")) {
    throw new Error(
      "Fusion child page HTML must not reference the review recording",
    );
  }

  console.log(
    "PASS: packaged-factories-index/fusion child reference browser verify",
  );
} catch (error) {
  console.error("Fusion child reference browser verify failed:");
  console.error(error);
  process.exitCode = 1;
} finally {
  cleanup();
