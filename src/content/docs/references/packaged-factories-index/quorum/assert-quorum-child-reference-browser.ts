/**
 * Browser verify for /docs/references/packaged-factories-index/quorum concise
...
const PORT = Number(process.env.QUORUM_CHILD_REFERENCE_PROBE_PORT ?? "3462");
const PAGE_PATH = "/docs/references/packaged-factories-index/quorum";
...
try {
  ...
  if (normalized.includes('"id": "builtin-quorum"')) {
    throw new Error("Child page must not embed unabridged factory.json");
  }

  if (normalized.includes("tts.factory-recording.v1.json")) {
    throw new Error(
      "Quorum child page HTML must not reference the tts recording",
    );
  }

  console.log(
    "PASS: packaged-factories-index/quorum child reference browser verify",
  );
} catch (error) {
  console.error("Quorum child reference browser verify failed:");
  console.error(error);
  process.exitCode = 1;
} finally {
  cleanup();
