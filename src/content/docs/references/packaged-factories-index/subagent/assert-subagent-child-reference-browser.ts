/**
 * Browser verify for /docs/references/packaged-factories-index/subagent concise
...
const PORT = Number(process.env.SUBAGENT_CHILD_REFERENCE_PROBE_PORT ?? "3522");
const PAGE_PATH = "/docs/references/packaged-factories-index/subagent";
...
try {
  ...
  for (const marker of required) {
    if (!normalized.includes(marker)) {
      ...
      const preview =
        previewIdx >= 0
          ? normalized.slice(previewIdx, previewIdx + 300)
  ...
  if (normalized.includes('"id": "builtin-subagent"')) {
    throw new Error("Child page must not embed unabridged factory.json");
  }

  if (normalized.includes("goal.factory-recording.v1.json")) {
    throw new Error(
      "Subagent child page HTML must not reference the goal recording",
    );
  }

  console.log(
    "PASS: packaged-factories-index/subagent child reference browser verify",
  );
} catch (error) {
  console.error("Subagent child reference browser verify failed:");
  console.error(error);
  process.exitCode = 1;
} finally {
  cleanup();
