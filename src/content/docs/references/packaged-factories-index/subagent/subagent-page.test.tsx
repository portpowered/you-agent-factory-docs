 ...
 * driven by the subagent recording. Cross-route recording import-graph
 * isolation lives in goal-subagent-recording-isolation.test.ts.
 */
  ...
  test(
    ...
    async () => {
      ...
      expect(fumadocsPage?.url).toBe(
        "/docs/references/packaged-factories-index/subagent",
      );
      ...
      expect(loadedPage.messages.title).toBe("@you/subagent");
      expect(loadedPage.messages.links?.canonicalName).toBe("@you/subagent");
      expect(loadedPage.messages.description).toMatch(/@you\/subagent/i);
      expect(
        String(loadedPage.messages.sections?.overview?.body ?? ""),
      ).toMatch(/packaged named factory/i);
      expect(
        String(loadedPage.messages.sections?.overview?.body ?? ""),
      ).toMatch(/one-pass subagent/i);
      expect(
        String(loadedPage.messages.sections?.overview?.body ?? ""),
      ...
      expect(canonical?.textContent).toContain("@you/subagent");
      ...
      expect(
        screen.getByText(/you run --named @you\/subagent --help/),
      ).toBeTruthy();

      const definitionLink = screen.getByRole("link", {
        name: /Complete @you\/subagent definition on Packaged Factory Reference/i,
      });
      ...
      // Distinctive subagent recording topology labels (not goal / siblings).
      expect(screen.getByText("run-subagent")).toBeTruthy();
      expect(screen.queryByText("execute-goal")).toBeNull();
    },
