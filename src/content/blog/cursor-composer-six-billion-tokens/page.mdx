...
When a long loop keeps every prior pass in the window, later turns pay for history that no longer helps. Prefer a smaller retained window that still carries the goal, the current diff, and the open failures over an ever-growing transcript that crowds out useful work.
