...
export const typedTaxonomyConsumerAuditContract: readonly TypedTaxonomyConsumerContractEntry[] =
  [
    ...
    },
    {
      ...
      rationale:
        "Glossary sidebar subgroup placement still checks canonical concept classification membership first with an editorial fallback. Factory Concepts, Program documentation, and Reference explorer subgroups are now registry-driven: each record declares its own placement in sidebarGrouping, and FACTORY_CONCEPTS_SIDEBAR_GROUP_BY_SLUG / FACTORY_DOCUMENTATION_SIDEBAR_MEMBERSHIP_BY_SLUG / FACTORY_REFERENCE_SIDEBAR_GROUP_BY_SLUG are generated from those records by scripts/generate-sidebar-grouping-runtime.ts rather than hand-maintained. The concepts redundancy check was retired with the hand-written map (the map derives from the field, so the two can no longer disagree); glossary redundancy is still reported because that placement resolves independently from classification membership. FAQ stays omitted as a top-level explorer page and W18 documentation move stubs stay omitted as compatibility-only routes; FACTORY_DOCUMENTATION_SIDEBAR_GROUP_BY_SLUG remains the top-group-only derived view.",
    },
