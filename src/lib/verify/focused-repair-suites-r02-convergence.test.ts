 ...
 * stay aligned for Concepts + eight Program documentation pages on the
 * combined tip. Suite paths live in
...
describe("focused repair suites R02 convergence", () => {
  ...
  test("search documents index Concepts and eight Program documentation pages", async () => {
  ...
  test("public sitemap lists Concepts and eight Program documentation pages", () => {
    const routes = new Set(listPublicSitemapRoutes());

    for (const url of [...R02_CONCEPTS_URLS, ...R02_PROGRAM_DOC_URLS]) {
