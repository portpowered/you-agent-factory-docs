import { describe, expect, test } from "bun:test";
import {
  SCAFFOLD_ID,
  SITE_BRAND_NAME,
  SITE_HEADING,
  SITE_PRODUCT_NAME,
} from "./scaffold";

describe("scaffold smoke", () => {
  test("exports a stable scaffold identifier for CI health checks", () => {
    expect(SCAFFOLD_ID).toBe("you-agent-factory-scaffold");
  });

  test("exports short header chrome mark and full product identity separately", () => {
    expect(SITE_BRAND_NAME).toBe("YOU");
    expect(SITE_PRODUCT_NAME).toBe("You Agent Factory");
    expect(SITE_HEADING).toBe("You Agent Factory");
    expect(SITE_HEADING).toBe(SITE_PRODUCT_NAME);
  });
});
