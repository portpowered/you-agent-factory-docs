...
describe("GLM-5 model page (glm-5-and-5-2-model-pages-002)", () => {
  test("registry record is published with GLM family metadata and GLM-5.2 cross-link", () => {
    const record = getModelById("model.glm-5");

    expect(record?.status).toBe("published");
    expect(record?.slug).toBe("glm-5");
    expect(record?.family).toBe("glm");
    expect(record?.releaseDate).toBe("2026-02-17");
    expect(record?.relatedIds).toContain("model.glm-5-2");
    expect(PUBLISHED_DOCS_REGISTRY_IDS.has("model.glm-5")).toBe(true);
  });

  test("curated related docs include the paired GLM-5.2 model record", () => {
    const source = getModelById("model.glm-5");
    if (!source) {
      throw new Error("expected model.glm-5 in registry");
    }
    ...
    expect(
      items.find((item) => item.registryId === "model.glm-5-2"),
    ).toBeDefined();
  });

  test("page renders title, folded opening summary, references, and GLM-5.2 link", async () => {
  ...
  test("local docs route loads published GLM-5 bundle", async () => {
    const page = await loadLocalDocsPage({ section: "models", slug: "glm-5" });
