/**
 * Consolidated review-facing slice proof for the Cosmos 3 model page.
...
const MODEL_URL = "/docs/models/cosmos-3";
...
describe("Cosmos 3 slice verification (cosmos-3-model-or-paper-page-005)", () => {
  ...
  test("app route metadata and English render resolve without missing-content placeholders", async () => {
    ...
    expect(metadata.title).toContain("Cosmos 3");
  ...
  test("rendered page exposes title, opening summary, metadata, tags, related docs, architecture graph, and references", async () => {
    ...
    expect(html).toContain("omnimodal world");
    expect(html).toContain("physical AI");
    expect(html).toContain('data-registry-id="model.cosmos-3"');
    expect(html).toContain("16B Nano and 64B Super family variants");
  ...
  test("representative search queries surface the canonical Cosmos 3 page", async () => {
  ...
  test("related-doc traversal exposes world-model and generation discovery paths", () => {
    ...
    expect(relatedHtml).toContain('href="/docs/glossary/world-model"');
    expect(relatedHtml).toContain('href="/docs/glossary/multimodal-model"');
    expect(relatedHtml).toContain('href="/docs/glossary/diffusion-model"');
  });

  test("served Cosmos 3 page renders title, sections, graph, tags, and references without errors", async () => {
    ...
    try {
      for (const viewport of [
        ...
        await page
          .getByRole("heading", { name: "Cosmos 3", exact: true })
