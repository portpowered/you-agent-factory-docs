...
import { GUIDES_DOCS_ROOT } from "@/lib/content/content-paths";
...
export async function loadGuidePageFromDisk(
  slug: string,
  locale: SiteLocale = defaultLocale,
  guidesDocsRoot = GUIDES_DOCS_ROOT,
): Promise<LoadedGuidePage> {
  const pageDir = join(guidesDocsRoot, slug);
  ...
  syncGraphRegistryForContentRoot(join(guidesDocsRoot, "..", ".."));
