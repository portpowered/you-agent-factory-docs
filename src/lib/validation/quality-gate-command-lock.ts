...
export const QUALITY_GATE_COMMAND_LOCK_DIR = ".quality-gate-command.lock";
const reentrantLockDepthByRepoRoot = new Map<string, number>();

function lockPath(repoRoot: string): string {
  return join(repoRoot, QUALITY_GATE_COMMAND_LOCK_DIR);
}
...
/** Waits for exclusive access to quality-gate subprocess execution. */
export function acquireQualityGateCommandLock(
  repoRoot: string,
  timeoutMs = 600_000,
): void {
  ...
  throw new Error(
    `Timed out after ${timeoutMs}ms waiting for quality-gate command lock at ${lockPath(repoRoot)}`,
  );
}

/** Releases the quality-gate command lock when held by the current process. */
export function releaseQualityGateCommandLock(repoRoot: string): void {
  const currentDepth = reentrantLockDepthByRepoRoot.get(repoRoot);
  if (currentDepth !== undefined) {
    if (currentDepth > 1) {
      reentrantLockDepthByRepoRoot.set(repoRoot, currentDepth - 1);
...
/** Runs a callback under an exclusive repo-root lock for quality-gate commands. */
export function withQualityGateCommandLock<T>(
  repoRoot: string,
  fn: () => T,
  timeoutMs = 600_000,
): T {
  acquireQualityGateCommandLock(repoRoot, timeoutMs);
  try {
    return fn();
  } finally {
    releaseQualityGateCommandLock(repoRoot);
  }
}
