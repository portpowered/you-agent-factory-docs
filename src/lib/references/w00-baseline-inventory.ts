/**
 * Derive W00 contract-inventory snapshots from the installed
 * `@you-agent-factory/api` package. Used by the fixture regenerator and
 * focused drift tests.
 *
 * Counts and membership lists are baseline observations for drift detection,
 * not permanent product limits or UI quotas.
 */
...
export const W00_BASELINE_OBSERVATION_NOTE =
...
export const W00_FIXTURE_FILE_NAMES = [
  "manifest-inventory.json",
  "openapi-inventory.json",
  "schema-inventory.json",
  "variant-inventory.json",
  "sse-inventory.json",
] as const;

export type W00FixtureFileName = (typeof W00_FIXTURE_FILE_NAMES)[number];
...
export function readInstalledOpenApi(): OpenApiDocument {
...
export function readInstalledManifest(): ManifestDocument {
  return require("@you-agent-factory/api/manifest") as ManifestDocument;
}

export function readInstalledFactorySchema(): JsonSchemaDocument {
  return require("@you-agent-factory/api/schemas/factory") as JsonSchemaDocument;
}

export function readInstalledYouConfigSchema(): JsonSchemaDocument {
  return require("@you-agent-factory/api/schemas/you-config") as JsonSchemaDocument;
}

export function readInstalledMockWorkersSchema(): JsonSchemaDocument {
  return require("@you-agent-factory/api/schemas/mock-workers") as JsonSchemaDocument;
}
...
export function buildManifestInventory(manifest: ManifestDocument) {
  ...
  return {
    baselineObservationNote: W00_BASELINE_OBSERVATION_NOTE,
...
export function buildOpenApiInventory(doc: OpenApiDocument) {
...
export function buildVariantInventory(factory: JsonSchemaDocument) {
...
export function buildSseInventory(doc: OpenApiDocument) {
...
export type W00BaselineInventories = {
  "manifest-inventory.json": ReturnType<typeof buildManifestInventory>;
  "openapi-inventory.json": ReturnType<typeof buildOpenApiInventory>;
  "schema-inventory.json": ReturnType<typeof buildSchemaInventory>;
  "variant-inventory.json": ReturnType<typeof buildVariantInventory>;
  "sse-inventory.json": ReturnType<typeof buildSseInventory>;
};

/** Recompute all five W00 inventory fixtures from the installed API package. */
export function deriveW00BaselineInventories(): W00BaselineInventories {
  const manifest = readInstalledManifest();
  const factory = readInstalledFactorySchema();
  const youConfig = readInstalledYouConfigSchema();
  const mockWorkers = readInstalledMockWorkersSchema();
  const openapi = readInstalledOpenApi();

  return {
    "manifest-inventory.json": buildManifestInventory(manifest),
    "openapi-inventory.json": buildOpenApiInventory(openapi),
    "schema-inventory.json": buildSchemaInventory(
      factory,
      youConfig,
      mockWorkers,
    ),
    "variant-inventory.json": buildVariantInventory(factory),
    "sse-inventory.json": buildSseInventory(openapi),
  };
}
