/**
 * Collect the transitive `#/components/schemas/...` closure reachable from
 * selected SSE payload roots. Pure document walk — no IO.
 *
 * Migrated from the W02 spike into the production events surface so later
 * inventory / optional AsyncAPI projection stories stay fail-closed on
 * unresolved refs without shipping spike routes.
 */

import type { EventsOpenApiComponentsSchemasLike } from "./openapi-document";

export type EventSchemaRefClosure = {
 ...
 * silently dropped.
 */
export function collectEventSchemaRefClosure(
  doc: EventsOpenApiComponentsSchemasLike,
  rootRefs: readonly string[],
): EventSchemaRefClosure {
