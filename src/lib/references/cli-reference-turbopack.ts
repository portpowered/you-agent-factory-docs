/**
 * Webpack-/Turbopack-safe CLI public-export resolution for Next.js server/docs
 * pages.
 *
 ...
 * walk (`resolveApiPackageManifestFsPath`), then join to `cli/commands.json`
 ...
 */
export function resolveCliReferenceFsPath(
...
/**
 * Resolve `@you-agent-factory/api/cli` (or the package manifest) to a `file:`
...
export function resolveCliReferenceExport(
  ...
  if (specifier !== CLI_REFERENCE_EXPORT_SPECIFIER) {
    throw new Error(
      `CLI reference Turbopack resolver only accepts "${CLI_REFERENCE_EXPORT_SPECIFIER}" or "${MANIFEST_EXPORT_SPECIFIER}", got "${specifier}".`,
    );
  }
