...
review. Match required GitHub **CI** / **verify** stages locally before review
(`make setup` → `check` → `test` → `build` → `budget` → `component-coverage`),
or use optional `make ci` when you need the broader local aggregate.
...
Required GitHub **CI** / **verify** (`.github/workflows/ci.yml`) runs
`make setup` → `check` → `test` → `build` → `budget` → `component-coverage` on
pull requests and pushes (see [README.md](../../README.md#quality-gates) and
[Deployment status expectations](../operations.md#deployment-status-expectations)).
Reproduce a failing stage locally with the same `make <target>`.

For a broader local aggregate before review (optional; includes linkcheck and
extra contracts beyond the required **verify** stages), run:
...
`make ci` is **not** what rewrite-era **CI** / **verify** invokes. Prefer the
Makefile stages above when matching the required GitHub check. Hosted PR
previews are **Deferred** — see [PR preview policy](../operations.md#pr-preview-policy).
...
and `make linkcheck`) catches registry and linking regressions early. Match the
required GitHub **CI** / **verify** stages locally (`make setup` → `check` →
`test` → `build` → `budget` → `component-coverage`) before opening the PR, or
run the broader optional `make ci` aggregate once when you need linkcheck and
extra contracts in one shot.
...
`validate-links.ts`, and build verifiers inside `make build`) are the supported
validation surface.
...
7. Match required **CI** / **verify** locally (`make setup` → `check` → `test` →
   `build` → `budget` → `component-coverage`), or run optional `make ci` once
...
Maintainer release and incident procedures live in
[operations.md](../operations.md). Use these section links rather than a generic
deployment-posture blurb:

| Concern | Runbook |
| --- | --- |
| Release on `main` | [Release process](../operations.md#release-process) |
| Prove which SHA is live on Pages | [Commit-SHA traceability](../operations.md#commit-sha-traceability) |
| Read-only live-site smoke (GET-only) | [Read-only post-deploy checks](../operations.md#read-only-post-deploy-checks) |
| Non-destructive recovery | [Rollback process](../operations.md#rollback-process) |
| Base-path / search / cache / stale artifact | [Incident diagnosis](../operations.md#incident-diagnosis) |
| What green checks mean on `main` vs PRs | [Deployment status expectations](../operations.md#deployment-status-expectations) |
| Hosted PR previews (Deferred) | [PR preview policy](../operations.md#pr-preview-policy) |

Live workflow display names: **CI** / **verify**; **Deploy GitHub Pages** /
**Canonical validation** / **Deploy to GitHub Pages**. File map:
