...
- React Flow docs graphs live in `src/components/docs/react-flow-diagram.tsx`; the wrapper projects checked-in nodes and edges into `@xyflow/react` with fit-view behavior and accessibility labels, while the rendered container owns live sizing so mobile hydration does not lock the graph to desktop dimensions.
