...
- When the docs entry route gains reviewer-visible fixtures that pull in new client libraries, recalibrate the checked-in `/docs` route and `_next/static` budgets in `src/lib/site-budget.ts` against the exported baseline rather than leaving `make budget` red on the reviewed head.
