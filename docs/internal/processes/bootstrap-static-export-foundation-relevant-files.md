...
- `validate:static-export` cleans `.next`, runs one production build, then invokes `tests/unit/static-export.test.ts` with `STATIC_EXPORT_SKIP_BUILD=1` so the served-export tests reuse that `out/` instead of starting a second back-to-back `next build`.
