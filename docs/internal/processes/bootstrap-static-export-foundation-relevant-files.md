...
- React Flow docs graphs live in `src/components/docs/react-flow-diagram.tsx`; the wrapper projects checked-in nodes and edges into `@xyflow/react` with accessibility labels, renders the graph at the authored viewport size, and scales that stage into the measured docs frame so desktop-to-mobile resize stays in bounds after hydration.
...
- When the homepage or docs entry route gains checked-in export-visible bytes after a rebase or reviewer-facing content/layout change, recalibrate the corresponding threshold in `src/lib/site-budget.ts` against the current exported baseline rather than leaving `make budget` red on the reviewed head.
