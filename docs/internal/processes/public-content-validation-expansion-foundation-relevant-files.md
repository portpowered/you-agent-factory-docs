...
- `src/lib/content/load-search-artifact.ts`
  - Parses the checked-in generated public search artifact from `public/search/public-search-index.json` and keeps disk IO separate from pure validation logic.
...
- `scripts/validate-content.ts`
  - Shared content-validation CLI entrypoint that runs starter-content validation, projects one canonical record per validated content path, and compares the checked-in search artifact against the real repository content graph.
- `src/lib/validation/gate-fixtures.ts`
  - Test-only fixture seam for subprocess proof that `validate:content` fails clearly on checked-in generated-artifact drift without swapping in a synthetic content graph.
