...
- In `.claude/worktrees/...` branches, Next.js can infer the workspace root from the parent repository and accidentally inherit a parent `postcss.config.mjs`. If the branch intentionally keeps plain CSS instead of the parent Tailwind stack, add a local `postcss.config.mjs` override so `make quality-gate`, `make build`, and export-backed tests stay branch-local and reproducible.
