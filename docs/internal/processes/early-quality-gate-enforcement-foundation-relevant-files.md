...
- `tests/unit/early-gate-automation-parity.test.ts` verifies `make quality-gate` delegates to `bun run quality-gate` and keeps the shared ordered step contract lightweight by asserting `EARLY_FOUNDATION_GATE_STEPS` directly instead of spawning a second full quality-gate subprocess inside the broader suite.
