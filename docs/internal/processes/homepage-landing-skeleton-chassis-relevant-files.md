...
- Production Wave B fill closeout (W-integrate batch-005): lane owns `src/app/(site)/compose-production-landing-slots.tsx` + `.test.tsx` (and may invert `production-home-landing-untouched.test.tsx` carousel/faq/cta placeholders when wiring those fills). Fence: do not edit `(dev)/landing-harness/**`, relocate `src/components/home`, rewrite `FactoryCarousel` / `FaqPanel` / `CtaBand` / `LandingPage` internals, or mix docs FAQ / footer / page-structure / SEO / graph-pages. Keep `renderHomePage` → `LandingPage {...composeProductionLandingSlots()}`. Quality gate: typecheck, lint, compose + untouched + docs-chrome-bypass tests; browser-verify `/` shows Wave A+B markers with no carousel/faq/cta placeholders and a docs route (e.g. `/docs/guides`) still mounts `#nd-sidebar`.
