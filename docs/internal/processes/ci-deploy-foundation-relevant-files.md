...
| `src/lib/seo/export-robots.ts` (+ tests) / `src/app/robots.ts` / `verify-export-seo-discovery.ts` (+ tests) | `out/robots.txt` production sitemap reference + composite SEO discovery export gate (canonicals/OG/social/alternates incl. `x-default` + blog English-only /sitemap/robots; hreflang lanes do not expand sitemap locale inventory) |
