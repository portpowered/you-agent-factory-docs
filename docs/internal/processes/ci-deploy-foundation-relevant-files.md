...
| `src/lib/content/content-runtime-preparation.ts` / `scripts/prepare-content-runtime.ts` / `bun run prepare:content-runtime` | Default prepare preserves valid `.source` and ignored generated runtime modules. Opt-in wipe: `--force-clean` or `CONTENT_RUNTIME_FORCE_CLEAN=1` (truthy: `1`/`true`/`yes`). Completeness still fails clearly when a required output is missing after preparation. |
