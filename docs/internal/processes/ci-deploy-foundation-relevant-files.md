...
| `src/lib/build/static-export-profile.test.ts` / `static-export-profile-diagnostics.test.ts` / `static-export-benchmark-prep.test.ts` / `static-export-compiler-cache.test.ts` / `static-export-legacy-compile-graph.test.ts` / `run-profiled-static-export.test.ts` / `bun run test:static-export-profile-contract` | Focused benchmark-contract gate: summary shape (stages, total, mode, cache reasons, scale counts, non-identifying machine metadata), optional-off default, clean/warm prep semantics (warm preserves `.next` compiler cache), legacy Atlas/AI compile-graph absence + factory route presence, stubbed stage runner — no full timed export required |
