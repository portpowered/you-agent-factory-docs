...
| `src/lib/i18n/route-locale.ts` | App-relative canonical + language alternates (`localizedRouteAlternates`); emits `languages["x-default"]` → English canonical whenever locale alternates are advertised; `englishOnlyCanonicalAlternates` for blog English-only policy until blog locales ship (`seo-hreflang`) |
