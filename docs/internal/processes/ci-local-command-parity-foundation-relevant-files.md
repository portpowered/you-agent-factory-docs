...
- Any test that deletes `.next/` or `tsconfig.tsbuildinfo` before `make check` or `next typegen` must take that same lock first, otherwise it can invalidate another suite's in-flight `make build` or export verification run.
- `src/lib/component-coverage/enforce.ts` defines the coverage subprocess scope; browser-driven suites that need Playwright-installed binaries must stay out of `make component-coverage` unless the CI lane explicitly provisions those browsers.
