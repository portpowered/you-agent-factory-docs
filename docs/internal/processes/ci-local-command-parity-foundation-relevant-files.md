...
- Local verification should not run `make check` in parallel with `bun test` against the same worktree: type generation, `.next/`, and `tsconfig.tsbuildinfo` are shared mutable artifacts, so concurrent runs can create false failures that do not reproduce in isolated CI-style execution.
