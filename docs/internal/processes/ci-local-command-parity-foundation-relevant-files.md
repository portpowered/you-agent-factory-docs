...
- Any helper that can reach `next build` through `make build`, `make quality-gate`, `scripts/quality-gate.ts`, or `validate:static-export` should take the static-export build lock as well as the repo-root command lock, otherwise one path can still race another on `.next/`, `out/`, or `tsconfig.tsbuildinfo`.
