...
import {
  discoverPlannerRootReconciliationDriftHandoffReport,
  formatPlannerRootReconciliationDriftHandoffReport,
  serializePlannerRootReconciliationDriftHandoffReport,
} from "../src/lib/factory/planner-root-reconciliation-drift-handoff";
...
const skipWorktreeDrift = process.argv.includes("--skip-worktree-drift");
const skipMergedLaneMetadata = process.argv.includes(
...
const reportedDriftAtUtc = readFlagValue("--reported-drift-at-utc");
...
const workListJsonText = workListPath
  ? readOptionalFile(workListPath, "work list")
  : skipLaneDiscovery && skipWorktreeDrift && skipMergedLaneMetadata
    ? undefined
    : readCompleteLiveWorkListSnapshotJson(repoRoot, [
        "work",
        "list",
...
const report = discoverPlannerRootReconciliationDriftHandoffReport({
  laneDiscoveryReport: skipLaneDiscovery ? undefined : discoverLaneReport(),
  remoteBaseRef,
  repoRoot,
  reportedDriftAtUtc,
  skipActivePrLinkage: skipLaneDiscovery,
  skipMergedLaneMetadata,
  skipWorktreeDrift,
  statusOutput,
  sessionListJsonText,
  workListJsonText,
...
process.stdout.write(
  isJsonOutputRequested(process.argv)
    ? serializePlannerRootReconciliationDriftHandoffReport(report)
    : `${formatPlannerRootReconciliationDriftHandoffReport(report)}\n`,
);
